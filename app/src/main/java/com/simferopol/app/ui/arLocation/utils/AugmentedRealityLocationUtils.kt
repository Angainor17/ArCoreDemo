...
object AugmentedRealityLocationUtils {
    ...
    private const val RENDER_MARKER_MAX_DISTANCE = 4000//meters
    ...
    fun handleSessionException(activity: Activity, sessionException: Exception) {
        val res = activity.resources
        val message = when (sessionException) {
            is UnavailableArcoreNotInstalledException ->
                res.getString(R.string.arcore_not_installed)
            is UnavailableUserDeclinedInstallationException ->
                res.getString(R.string.arcore_not_installed)
            is UnavailableApkTooOldException ->
                res.getString(R.string.arcore_not_updated)
            is UnavailableSdkTooOldException ->
                res.getString(R.string.arcore_not_supported)
            else -> res.getString(R.string.arcore_not_supported)
        }
        Toast.makeText(activity, message, Toast.LENGTH_LONG).show()
    }

    fun getScaleModifierBasedOnRealDistance(distance: Int): Float = when (distance) {
    ...
    fun generateRandomHeightBasedOnDistance(distance: Int): Float = when (distance) {
